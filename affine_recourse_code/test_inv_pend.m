...
for j = 1:batch
    ...
    for i = 1:epoch
        ...
        while ~(strcmp(status,'Solved') || strcmp(status,'Inaccurate/Solved'))
            [F,u_0,status] = affine_recourse_inv_pend(A,B,Bw,x_01,x_tar,n,T,mu,Sigma,eta*(1.281551565545)^0.5,gamma);
        end
        [x_1,x_2,u_res] = propagate_dynamics_inv_pend(A,B,Bw,T,u,x_01,x_02,F,u_0,mu,Sigma);
        X_1 = [X_1;x_1(n+1:end,:)];
        X_2 = [X_2;x_2(n+1:end,:)];
        U_res = [U_res;u_res];
        status = '';
